...
<body>
<div id="container">
    <p><a href="bookapplite.php">Food</a> | <a href="organic.php">Organic</a> | <a href="cash.php">Cash</a> | <a id="activeTab"  href="misc.php">Магаз</a> | <a href="cat0.php">Cat0</a></p>
    <div id="addItemForm">
        <form method="post">
    ...
    <?php
        if(isset($_SESSION['id'])) {
            ...
            if(isset($_POST['removeID'])) {
                $query = "DELETE FROM misc WHERE id = ?";
                $statement = $db->prepare($query);
                $statement->execute(array($_POST['idToDelete']));
            }

            if(isset($_POST['addBook'])) {
                $query = "INSERT INTO misc(name) VALUES(?)";
                $statement = $db->prepare($query);
                $statement->execute(array($_POST['name']));
            }

            $statement = $db->query("SELECT id, name FROM misc");
            echo "<table>";
