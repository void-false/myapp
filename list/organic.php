...
<body>
<div id="container">
    <p><a href="bookapplite.php">Food</a> | <a id="activeTab"  href="organic.php">Organic</a> | <a href="cash.php">Cash</a> | <a href="misc.php">Магаз</a> | <a href="cat0.php">Cat0</a></p>
    <div id="addItemForm">
        <form method="post">
    ...
    <?php
        if(isset($_SESSION['id'])) {
            ...
            $db->exec("CREATE TABLE IF NOT EXISTS organic(id INTEGER PRIMARY KEY, name TEXT)");

            if(isset($_POST['removeID'])) {
                $query = "DELETE FROM organic WHERE id = ?";
                $statement = $db->prepare($query);
                $statement->execute(array($_POST['idToDelete']));
            }

            if(isset($_POST['addBook'])) {
                $query = "INSERT INTO organic(name) VALUES(?)";
                $statement = $db->prepare($query);
                $statement->execute(array($_POST['name']));
            }

            $statement = $db->query("SELECT id, name FROM organic");
            echo "<table>";
